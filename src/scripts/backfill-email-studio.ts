...
for (const row of generated) {
  ...
  const email = { project_id: projectId, contact_id: row.contact_id, company_id: pc?.company_id ?? contact.company_uuid ?? null, contact_name: String(contact.name ?? `${contact.first_name ?? ""} ${contact.last_name ?? ""}`.trim()), company_name: String(contact.company_name ?? ""), campaign_id: campaignId, batch_name: batchName, persona: String(ctx.persona ?? contact.position ?? ""), sequence_step: step, recipient_email: String(contact.work_email ?? contact.email ?? contact.personal_email ?? "").toLowerCase() || null, current_subject: subject, current_body: content, current_model: String(ctx.model ?? "") || null, research_quality: "unknown", status: "ai_draft_made", provenance: "voitech_generated", generation_history_available: true };
  if (!apply) { generatedImported++; continue; }
  const up = await client.from("outreach_emails").upsert(email, { onConflict: "project_id,contact_id,campaign_id,batch_name,sequence_step", ignoreDuplicates: true }).select("*").maybeSingle(); if (up.error) throw new Error(up.error.message); if (!up.data) { generatedSkipped++; continue; }
