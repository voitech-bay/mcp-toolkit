...
async function generateDraft(client: NonNullable<ReturnType<typeof getSupabase>>, email: Json, prompt: string, previous?: Json, comments: Json[] = [], includedResearchPointIds: string[] = [], styleSourceId?: unknown) {
  ...
  const call = await structuredCall({ model, schema: EmailDraftSchema, trace: { feature: "email-studio", stage: previous ? "regenerate" : "generate", project_id: email.project_id, contact_id: email.contact_id },
    system: systemPrompt,
    // active_knowledge leads and per-contact fields trail so the stable, cacheable
    // block sits at the front of the user message prefix (see Phase 8 of the
    // Wellore integration plan).
    user: JSON.stringify({ active_knowledge: knowledge.documents, task: `${prompt}${anchorDirective}${styleDirective}`, recipient_context: context, research_points: anchoredAllowed, priority_anchors: priorityAnchors, selected_style_source: styleSource, previous_version: previous ?? null, unresolved_comments: comments.map((c) => ({ id: c.id, selected_quote: c.selected_quote, body: c.body })), required_comment_ids: comments.map((c) => c.id), sequence_step: sequenceStep, step_number: sequenceStep }) });
  ...
  // Surfaced separately from the raw usage blob so cached-prefix savings are
  // directly visible on the version record rather than buried in nested JSON
  // (Phase 8 of the Wellore integration plan).
  const cachedTokens = parseTokenUsage((call.usage as Json) ?? {}).cached_tokens;
  return { draft: call.value, validation, model, knowledgeManifest: knowledge.manifest, research, usage: call.usage, cachedTokens, priorityAnchors, styleSource };
}
