...
const resultsLabel = computed(() => {
  const msgs = total.value;
  const contacts = contactGroups.value.length;
  if (msgs === 0) return loading.value ? "Loading…" : "0 messages match filters";
  const start = (page.value - 1) * pageSize.value + 1;
  const end = Math.min(page.value * pageSize.value, msgs);
  const contactWord = contacts === 1 ? "contact" : "contacts";
  return `Showing ${start}–${end} of ${msgs} messages · ${contacts} ${contactWord} on this page`;
});
