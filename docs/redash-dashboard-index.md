...
| 53 | VT / Flow detail — pipeline stage rates per flow | FlowLeads + Flows + Contacts + PipelineStages | TABLE (viz **86**, widget **71**): exact `PipelineStages.name` → buckets (Approaching, Engaging, Replied + Replied - Negative, Replied - Positive, Opportunity + Active Opportunity, Not Interested + Do Not Contact); includes query dropdown parameter `rate_metric` (`reply`, `reply_positive`, `opportunity`) with `selected_rate_pct`/`selected_rate_count` for chart switching; keeps `n_engaging` / `engaging_rate_pct`; `n_other` / `n_no_stage` for QA — block **4c** |
