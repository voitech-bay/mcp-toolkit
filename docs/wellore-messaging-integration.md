...
- `src/scripts/seed-wellore-outreach-knowledge.ts` (`npm run seed:wellore-outreach`) — checksum-versioned `project_knowledge_documents` rows (voice rules, service catalog, cases, examples, lane framework). Re-run after editing `ai-toolkit/.cursor/skills/wellore-email-copywriting/SKILL.md` or the seed script's own inline content.
